...
class WP_Taxi_Customer_Dashboard_Shortcode {
    ...
    public static function init() {
        // API Key und Preise aus den Admin-Einstellungen laden
        self::$google_maps_api_key = get_option('wp_taxi_plugin_google_maps_api_key', '');
        // Fallback, falls der Key im Prompt absolut verwendet werden soll und nicht aus Settings.
        // Für eine produktive Umgebung sollte der Key immer aus den Settings kommen.
        if (empty(self::$google_maps_api_key)) {
            self::$google_maps_api_key = 'AIzaSyBbWGNbH7yEXc7mtQrwvQEPmghYfr9-glI'; // Fallback zum Prompt-Key
        }
        ...
        <div id="wp-taxi-customer-dashboard">
            ...
            <div id="booking-form">
                ...
                <div id="route-info" style="margin-bottom: 15px;">
                    ...
                    <p><?php _e('Geschätzter Preis:', 'wp-taxi-plugin'); ?> CHF <span id="route-price">---</span></p>
                </div>
    ...
    public static function display_ride_history() {
        ...
        // $table_name = $wpdb->prefix . 'taxi_rides'; // Wird später in der Installer-Klasse erstellt

        // Dummy-Daten für jetzt, da die Tabelle noch nicht existiert
        // if ($wpdb->get_var("SHOW TABLES LIKE '$table_name'") != $table_name) {
            echo '<p>' . __('Ihre Fahrtenhistorie wird hier angezeigt, sobald Sie Fahrten gebucht haben.', 'wp-taxi-plugin') . '</p>';
            echo '<p>' . __('(Datenbanktabelle für Fahrten muss noch erstellt werden)', 'wp-taxi-plugin') . '</p>';
            return;
        // }
        ...
        if ( $rides ) {
            ...
            foreach ( $rides as $ride ) {
                echo '<li>';
                echo sprintf(
                    __('Fahrt von %s nach %s am %s - Status: %s', 'wp-taxi-plugin'),
                    esc_html($ride->start_address),
                    esc_html($ride->end_address),
                    date_i18n( get_option( 'date_format' ) . ' ' . get_option( 'time_format' ), strtotime( $ride->created_at ) ),
                    esc_html(self::get_status_translation($ride->status))
                );
                if ($ride->price) {
                    echo ' - ' . sprintf(__('Preis: CHF %.2f', 'wp-taxi-plugin'), $ride->price);
                }
                echo '</li>';
            }
            echo '</ul>';
    ...
    public static function ajax_request_ride() {
        ...
        $ride_data = array(
            ...
            'duration'      => isset($_POST['duration']) ? sanitize_text_field($_POST['duration']) : null,
        );

        $data_formats = array(
            '%d', '%s', '%s', '%s', '%s', '%s', '%s', '%s', '%f', '%s', '%s', '%f', '%s'
        );

        $result = $wpdb->insert( $table_name, $ride_data, $data_formats );

        if ( $result ) {
            $ride_id = $wpdb->insert_id;
            // Aktion für neue Fahrtanfrage auslösen
            do_action( 'wp_taxi_ride_requested', $ride_id, $ride_data );

            wp_send_json_success( array(
                'message' => __( 'Fahrt erfolgreich angefragt. Ein Fahrer wird in Kürze benachrichtigt.', 'wp-taxi-plugin' ),
                'ride_id' => $ride_id
            ) );
        } else {
